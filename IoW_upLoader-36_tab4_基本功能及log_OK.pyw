...
class BatchLikeApp:
        ...
        try:
            ...
            if os.path.exists(self.log_file):
                ...
                if file_size > self.max_log_size:
                    ...
                    # 记录备份事件到 RAM 日志（此时RAM日志系统已可用）
                    backup_msg = f"旧日志文件超过 {self.max_log_size/1024}KB，已备份至 {backup_file}"
                    self.log_to_file(backup_msg, "SYSTEM")
                    
            # 重要：不再创建或打开 self.log_file 进行写入！
            print("系统提示: 日志系统运行在纯内存模式。")
            print("        如需保存日志，请在日志显示区域按右键选择『另存新档』。")
            
        except Exception as e:
            error_msg = f"初始化日志系统时发生错误: {e}"
            print(error_msg)
    ...
    def create_log_frame(self):
        ...
        self.context_menu.add_command(label="快速匯出 (自動命名)", command=self.quick_export_log)
    ...
    def log_to_file(self, message, level="INFO"):
        """【新】将日志记录到RAM缓冲区，不写入实体文件"""
        try:
            # 1. 生成时间戳和日志条目
            timestamp = datetime.now().strftime('[%Y-%m-%d %H:%M:%S]')
            log_entry = f"{timestamp} [{level}] {message}"
            
            # 2. 计算这条日志占用的字节大小 (UTF-8编码)
            entry_size = len(log_entry.encode('utf-8'))
            
            # 3. 【关键】检查缓冲区大小并执行FIFO策略（超过100KB删最旧的）
            if self.current_buffer_size + entry_size > self.max_log_buffer_size:
                # 删除最旧的日志直到有足够空间
                while self.ram_log_buffer and (self.current_buffer_size + entry_size > self.max_log_buffer_size):
                    removed_entry = self.ram_log_buffer.pop(0) # 移除列表第一个（最旧）
                    removed_size = len(removed_entry.encode('utf-8'))
                    self.current_buffer_size -= removed_size
            
            # 4. 添加新日志到内存缓冲区并更新大小计数
            self.ram_log_buffer.append(log_entry)
            self.current_buffer_size += entry_size
            
            # 5. 更新UI显示（调用一个我们稍后会创建的方法）
            self.update_log_display(log_entry)
            
        except Exception as e:
            # 万一RAM日志系统失败，至少打印到控制台供调试
            print(f"[CRITICAL] 内存日志记录失败: {e}")
    ...
    def update_log_display(self, log_entry):
        ...
        if threading.current_thread() is threading.main_thread():
            self._do_update_log_display(log_entry)
        else:
            # 从非主线程调用，使用 after 方法调度到主线程
            self.root.after(0, self._do_update_log_display, log_entry)

    def _do_update_log_display(self, log_entry):
        """实际更新日志显示的内部方法"""
        # 检查 log_text 控件是否存在（可能UI还没初始化）
    ...
    def export_log_to_file(self):
        """将RAM中的日志匯出为文件"""
        if not self.ram_log_buffer:
            messagebox.showinfo("資訊", "日誌緩衝區為空，無內容可匯出。")
            return
        
        # 弹出文件选择对话框让用户指定保存位置和文件名
        ...
        if file_path:  # 如果用户没有取消对话框
            try:
                # 将内存缓冲区中的所有日志条目写入文件
                with open(file_path, 'w', encoding='utf-8') as f:
                    for log_entry in self.ram_log_buffer:
                        f.write(log_entry + '\n')
                
                # 【可选】记录本次匯出操作到日志（使用新的内存日志系统）
                self.log_to_file(f"日誌已成功匯出到: {file_path}", "SYSTEM")
                messagebox.showinfo("成功", f"日誌已匯出到:\n{file_path}")
                
            except Exception as e:
                # 如果保存失败，记录错误并提示用户
                error_msg = f"匯出日誌檔案失敗: {e}"
                self.log_to_file(error_msg, "ERROR")
                messagebox.showerror("錯誤", error_msg)

    def quick_export_log(self):
        """快速匯出日志：自动命名并保存到程序所在目录"""
        if not self.ram_log_buffer:
            messagebox.showinfo("資訊", "日誌緩衝區為空，無內容可匯出。")
            return

        try:
            # 1. 自动生成文件名 (例如: log_20241027_023456.txt)
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            default_filename = f"log_{timestamp}.txt"
            
            # 2. 保存到程序所在的目录 (与.py文件同一文件夹)
            #    如果你希望固定保存到桌面，可改为：
            #    import os.path
            #    desktop_path = os.path.join(os.path.expanduser("~"), "Desktop")
            #    file_path = os.path.join(desktop_path, default_filename)
            program_dir = os.path.dirname(os.path.abspath(__file__))
            file_path = os.path.join(program_dir, default_filename)
            ...
            # 4. 记录日志并提示用户完整路径
            success_msg = f"日誌已快速匯出至: {file_path}"
            self.log_to_file(success_msg, "SYSTEM")
            
            # 显示提示信息（可稍后自动关闭）
            messagebox.showinfo("快速匯出成功", 
                               f"日志已保存到：\n{file_path}\n\n（此提示3秒后自动关闭）")
            
        except Exception as e:
            error_msg = f"快速匯出失敗: {e}"
            self.log_to_file(error_msg, "ERROR")
            messagebox.showerror("錯誤", error_msg)
    ...
    def _calculate_file_hash(self, file_path, algorithm='md5'):
        ...
        try:
            ...
            self.log_message(f"計算檔案哈希值時出錯 {file_path}: {e}", level="ERROR")
            return None
